...
class FirestoreCRUD:
    ...
    def get_recommendation_data(self, mac_address: str):
        ...
        try:
            ...
            user = None
            user_doc_ref = None  # To hold the document reference for accessing subcollections
            ...
            orders = orders_ref.order_by('Date', direction=firestore.Query.DESCENDING).limit(1).stream()
            ...
            if last_order:
                ...
                if meals_in_last_order:
                    ...
                    most_ordered_meal_last = max(meals_in_last_order, key=meals_in_last_order.get)

                    # Fetch meal details
                    meal_ref = self.db.collection('Meals').where('name', '==', most_ordered_meal_last).limit(1).stream()
                    meal = None
                    for m in meal_ref:
                        meal = m.to_dict()
                        break
                    if meal:
                        ...
                        recommendations.append({'name': 'Last ordered meal not found.', 'image': ''})
                else:
                    recommendations.append({'name': 'No meals found in the last order.', 'image': ''})
            else:
                recommendations.append({'name': 'No previous orders found.', 'image': ''})

            # 2. Age and Allergy-Based Recommendation
            if age <= 12:
                kids_meals = self.db.collection('Meals').where('category', '==', 'Kids').stream()
                ...
                if kids_meals_list:
                    ...
                    recommendations.append({'name': 'No Kids meals available.', 'image': ''})
            else:
                # Determine the group based on allergies
                if is_diabetic:
                    # Query users where 'allergies.diabetic' is True
                    group_query = users_ref.where('allergies.diabetic', '==', True).stream()
                elif is_gluten:
                    # Query users where 'allergies.gluten' is True
                    group_query = users_ref.where('allergies.gluten', '==', True).stream()
                else:
                    # Query users where both 'allergies.diabetic' and 'allergies.gluten' are False
                    group_query = users_ref.where('allergies.diabetic', '==', False).where('allergies.gluten', '==', False).stream()
                ...
                for group_user in group_query:
                    ...
                    for order in group_orders:
                        ...
                        for meal_name, count in meals.items():
                            meal_counts[meal_name] = meal_counts.get(meal_name, 0) + count

                if meal_counts:
                    # Find the most ordered meal
                    most_ordered_meal = max(meal_counts, key=meal_counts.get)
                    # Fetch meal details
                    meal_ref = self.db.collection('Meals').where('name', '==', most_ordered_meal).limit(1).stream()
                    meal = None
                    for m in meal_ref:
                        meal = m.to_dict()
                        break
                    if meal:
                        ...
                        recommendations.append({'name': 'Recommended meal not found.', 'image': ''})
                else:
                    recommendations.append({'name': 'No data available for recommendation.', 'image': ''})

            # 3. Breakfast Category Recommendation
            breakfast_meals = self.db.collection('Meals').where('category', '==', 'Breakfast').stream()
            ...
            for user_doc in users_ref.stream():
                ...
                for order in user_orders:
                    ...
                    for meal_name, count in meals.items():
                        ...
                        if any(meal.get('name') == meal_name and meal.get('category') == 'Breakfast' for meal in breakfast_meals_list):
                            breakfast_counts[meal_name] = breakfast_counts.get(meal_name, 0) + count

            if breakfast_counts:
                most_ordered_breakfast = max(breakfast_counts, key=breakfast_counts.get)
                # Fetch meal details
                meal_ref = self.db.collection('Meals').where('name', '==', most_ordered_breakfast).limit(1).stream()
                meal = None
                for m in meal_ref:
                    meal = m.to_dict()
                    break
                if meal:
                    ...
                    recommendations.append({'name': 'Breakfast meal not found.', 'image': ''})
            else:
                recommendations.append({'name': 'No Breakfast data available.', 'image': ''})

            # 4. Lunch Category Recommendation
            lunch_meals = self.db.collection('Meals').where('category', '==', 'Lunch').stream()
            ...
            for user_doc in users_ref.stream():
                ...
                for order in user_orders:
                    ...
                    for meal_name, count in meals.items():
                        ...
                        if any(meal.get('name') == meal_name and meal.get('category') == 'Lunch' for meal in lunch_meals_list):
                            lunch_counts[meal_name] = lunch_counts.get(meal_name, 0) + count

            if lunch_counts:
                most_ordered_lunch = max(lunch_counts, key=lunch_counts.get)
                # Fetch meal details
                meal_ref = self.db.collection('Meals').where('name', '==', most_ordered_lunch).limit(1).stream()
                meal = None
                for m in meal_ref:
                    meal = m.to_dict()
                    break
                ...
                if meal:
                    ...
                    recommendations.append({'name': 'Lunch meal not found.', 'image': ''})
            else:
                recommendations.append({'name': 'No Lunch data available.', 'image': ''})
    ...
    def update_emotions(self, mac_address: str, page_id: int, expressions_array: list):
        """
        Updates the expressions for a specific page (page_id) for a user identified by their macAddress.
        
        Parameters:
            mac_address (str): The MAC address of the user.
            page_id (int): The ID of the page (1, 2, 3, or 4).
            expressions_array (list): New expressions to append for the specified page.

        Returns:
            dict: A dictionary containing a success message or an error message.
        """
        try:
            # Ensure page_id is within valid range
            if page_id not in [1, 2, 3, 4]:
                return {"error": "Invalid page_id. Must be one of 1, 2, 3, or 4."}
            ...
            current_page4 = user_data.get('page4expressions', [])

            # Determine which page to update
            if page_id == 1:
                current_page1 += expressions_array
                updated_field = 'page1expressions'
            elif page_id == 2:
                current_page2 += expressions_array
                updated_field = 'page2expressions'
            elif page_id == 3:
                current_page3 += expressions_array
                updated_field = 'page3expressions'
            else:  # page_id == 4
                current_page4 = current_page4 + expressions_array
                updated_field = 'page4expressions'

            # Update the Firestore document with the updated expressions array for the correct page
            update_data = {updated_field: locals()[f'current_page{page_id}']}
            user_doc_ref.update(update_data)

            return {"message": f"Emotions for page {page_id} updated successfully."}
