...
def setup_server(host='10.10.159.84', port=9000):
...
# Global variables to store previous values of gesture, finger_gesture, and hand
previous_gesture = None
previous_finger_gesture = None
previous_hand = None

def send_data_to_client(client_socket, gesture, finger_gesture, hand):
    global previous_gesture, previous_finger_gesture, previous_hand
    
    # Only send data if any of the variables changed
    if gesture != previous_gesture or finger_gesture != previous_finger_gesture or hand != previous_hand:
        # Data has changed, so prepare to send
        data = {
            'operation': 'MediaPipe',
            'data': {
                'gesture': gesture,
        ...
        print(f"Sent data to client: {json_data}")
        
        # Update the previous values
        previous_gesture = gesture
        previous_finger_gesture = finger_gesture
        previous_hand = hand
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Send the gesture data to the client
